...
APP_VERSION = "15.0-persistent-control-tower"
...
def _master_with_operational_overlay(master):
    ...
    # v15.7: Quantity-aware Return -> TEI -> CN workflow.
    def tei_cn_pending_remark(r):
        ...
        # The source return quantity is preferred because CN Qty may not exist yet.
        return_qty = abs(num(r.get("courier_customer_return_qty")))
        if return_qty <= 0:
            return_qty = abs(num(r.get("cn_qty")))
        if return_qty <= 0 and txt(r.get("order_status")).lower() == "returned":
            return_qty = max(abs(num(r.get("order_qty"))), 1.0)

        # v15.11 takes precedence over TEI/CN aging.
        # Physical return received + no invoice/billing + no payment/refund
        # means the order is already reconciled and no TEI/CN task is required.
        no_billing = (
            abs(num(r.get("invoice_qty"))) <= 0.000001
            and not txt(r.get("invoice_no"))
        )
        no_payment = (
            abs(num(r.get("net_pay_received"))) <= 0.000001
            and abs(num(r.get("refund"))) <= 0.000001
        )
        if (
            return_qty > 0
            and pd.notna(return_date)
            and no_billing
            and no_payment
        ):
            return "Reconciled"

        raw_tei_qty = num(r.get("tei_qty"))
        # Backward-compatible conservative default:
        # a TEI number with blank/zero TEI Qty represents only one covered unit.
        tei_covered_qty = raw_tei_qty
        if tei_no and tei_covered_qty <= 0:
            tei_covered_qty = 1.0

        # Return received for 2 days but no TEI created.
        if return_qty > 0 and pd.notna(return_date) and not tei_no and pd.isna(tei_date):
            age_from_return = (pd.Timestamp(date.today()) - return_date.normalize()).days
            if age_from_return >= 2:
                return "Return Received TEI Pending"

        # A TEI exists, but it does not cover the full returned quantity.
        if return_qty > 0 and tei_no and tei_covered_qty + 0.000001 < return_qty:
            return "Partial TEI Generated - Balance TEI Pending"

        # Full TEI coverage reached; CN becomes the next open step.
        if return_qty > 0 and tei_no and tei_covered_qty + 0.000001 >= return_qty:
            if cn_no or not pd.isna(cn_date):
                return current
            if pd.notna(tei_date):
                age_days = (pd.Timestamp(date.today()) - tei_date.normalize()).days
                if age_days >= 5:
                    return "TEI Generated but CN Pending"
            return "CN Pending"

        # Preserve legacy CN-aging behavior for non-return cases.
        if "cn pending" in current.lower() and tei_no and pd.notna(tei_date):
            if not cn_no and pd.isna(cn_date):
                age_days = (pd.Timestamp(date.today()) - tei_date.normalize()).days
                if age_days >= 5:
                    return "TEI Generated but CN Pending"
...
repair_payment_status_consistency()
...
repair_returned_unbilled_no_payment()
...
if workspace == "E-Com Reconciliation Dashboard":
    ...
    if is_admin_unlocked():
        ...
        if files and st.button("Run & Save Reconciliation",type="primary",use_container_width=True):
            ...
            # v15.8: force one clean rerun after successful source save so all
            # cached dashboard views reopen from the newly persisted database.
